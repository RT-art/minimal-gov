...
variable "share_principal_account_id" {
  type        = string
  description = "AWS account ID (12 digits) that should receive the TGW RAM share"
}
...
data "terraform_remote_state" "workload" {
  ...
  config = {
    path = "${path.module}/../../workload/test/terraform.tfstate"
  }
}

locals {
  workload_tgw_attachment_id = data.terraform_remote_state.workload.outputs.tgw_attachment_id
}
...
resource "aws_security_group" "instance" {
  ...
  ingress {
    description = "ICMP from workload VPC"
    from_port   = -1
    to_port     = -1
    protocol    = "icmp"
...
resource "aws_ec2_transit_gateway" "this" {
  ...
  tags                            = merge(local.tags, { Name = "${local.name_prefix}-tgw" })
}

resource "aws_ec2_transit_gateway_route_table" "this" {
  transit_gateway_id = aws_ec2_transit_gateway.this.id
  tags               = merge(local.tags, { Name = "${local.name_prefix}-tgw-rt" })
}

resource "aws_ram_resource_share" "tgw" {
  name                      = "${local.name_prefix}-tgw-share"
  allow_external_principals = true
  tags                      = merge(local.tags, { Name = "${local.name_prefix}-tgw-share" })
}
...
resource "aws_ram_principal_association" "share_target" {
  resource_share_arn = aws_ram_resource_share.tgw.arn
...
resource "aws_ec2_transit_gateway_route_table_association" "workload" {
  transit_gateway_attachment_id  = local.workload_tgw_attachment_id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.this.id
  depends_on                     = [aws_ram_principal_association.share_target]
}
